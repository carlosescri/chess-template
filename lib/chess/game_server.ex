defmodule Chess.GameServer do
  ...
  def get_availables_moves(game_name, nil) do
    []
  end

  def get_availables_moves(game_name, selected_figure) do
    GenServer.call(game_name, {:get_availables_moves, selected_figure})
  end
  ...
  def handle_call(:join, from, state) do
    ...
    case Game.join(pid, state) do
      {:ok, new_state, player} ->
        {:reply, {:ok, new_state, player}, new_state}
  ...
  def handle_call({:move, figure, new_position}, _from, state) do
    new_state =
  ...
  def handle_call({:get_availables_moves, selected_figure}, _from, state) do
    moves = Game.get_availables_moves(state, selected_figure)
    {:reply, moves, state}
  end
