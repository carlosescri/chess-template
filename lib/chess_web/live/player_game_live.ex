defmodule ChessWeb.PlayerGameLive do
  ...
  def handle_event(
        "select",
        _params,
        %{assigns: %{player: :white, game_state: %{turn: :black}}} = socket
      ) do
    {:noreply, socket}
  end
  ...
  def handle_event(
        "select",
        _params,
        %{assigns: %{player: :black, game_state: %{turn: :white}}} = socket
      ) do
    {:noreply, socket}
  end
  ...
  def handle_event(
        ...
        %{assigns: %{selected_cell: {0, 0}}} = socket
      ) do
    ...
    available_moves =
      GameServer.get_availables_moves(socket.assigns.game_name, selected_figure)

    cond do
      is_nil(selected_figure) ->
        {:noreply, socket}
  ...
  def handle_event("select", %{"column" => column, "row" => row}, socket) do
    ...
    if {row, column} in socket.assigns.available_moves do
      {:ok, new_state} =
        GameServer.move(socket.assigns.game_name, socket.assigns.selected_figure, {row, column})
  ...
  def get_figure(row, column, game_state) do
    figure =
      Enum.find(game_state.figures, fn %Chess.Figures{position: {figure_row, figure_column}} ->
        {figure_row, figure_column} == {row, column}
      end)
