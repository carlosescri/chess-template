defmodule ChessWeb.GameLive do
  ...
  def handle_event(
        "click:square",
        %{"square" => square},
        %{assigns: %{first_square_clicked: first_square_clicked, board: board, role: role}} =
          socket
      ) do
    cond do
      get_player_color(socket) == Board.square_color(board, square) ->
        socket =
          socket
      ...
      Board.enemy_piece?(board, square, role) ->
